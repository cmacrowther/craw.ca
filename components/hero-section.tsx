...
export function HeroSection() {
  ...
  let originalColors: THREE.Color[] = [];
    ...
    const baseHue = isLightTheme ? 0 : 0.6; // Black-ish for light theme, blue-ish for dark theme
    const baseSaturation = isLightTheme ? 0 : 0.6; // No saturation for black, full color for dark theme
    const baseLightness = isLightTheme ? 0.1 : 0.65; // Dark particles for light theme, bright for dark theme
    ...
    if (threeRef.current) {
      ...
      for (let ix = 0; ix < AMOUNTX; ix++) {
        for (let iy = 0; iy < AMOUNTY; iy++) {
          ...
          if (isLightTheme) {
            // Dark particles for light theme - varying shades of dark gray/black
            const lightness = baseLightness + 0.1 * Math.sin(iy / AMOUNTY * Math.PI);
            color = new THREE.Color().setHSL(0, 0, Math.max(0.05, lightness));
          } else {
            // Colorful particles for dark theme - blue to purple to pink gradient
            const t = ix / AMOUNTX;
      ...
      const handleMouseMove = (e: MouseEvent) => {
        ...
        // Only enable effect if mouse is in the lower 50% of the area
        const relY = (e.clientY - rect.top) / rect.height;
        if (relY >= 0.5 && relY <= 1) {
          isMouseOver = true;
          // Normalize mouse position to [-1, 1] within the area
          targetMouseX = ((e.clientX - rect.left) / rect.width) * 2 - 1;
          targetMouseY = (relY - 0.5) * 4 - 1; // Remap [0.5,1] to [-1,1]
          mouseX = (e.clientX - rect.left) / rect.width;
          mouseY = (relY - 0.5) * 2; // Remap [0.5,1] to [0,1]
        } else {
          isMouseOver = false;
        }
      };
  ...
  targetFov = 100 + Math.min(16, scrollY * 0.032);
      ...
      function animate() {
        ...
        // If not over, smoothly interpolate to neutral (0,0)
        if (!isMouseOver) {
          targetMouseX = 0;
          targetMouseY = 0;
        }
        ...
        for (let ix = 0; ix < AMOUNTX; ix++) {
          for (let iy = 0; iy < AMOUNTY; iy++) {
            ...
            // Subtle color shift around mouse (lower intensity)
            if (isMouseOver) {
              // Calculate distance from mouse to particle in normalized [0,1] space
              const px = ix / AMOUNTX;
              const py = iy / AMOUNTY;
              const dist = Math.sqrt((px - mouseX) ** 2 + (py - mouseY) ** 2);
              // If close to mouse, shift hue slightly
              if (dist < 0.18) {
                if (isLightTheme) {
                  // For light theme, slightly lighten the dark particles near mouse
                  ...
                  // For dark theme, shift hue as before
        ...
        // Only scroll affects camera now
  ...
  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[75%] w-full max-w-4xl text-left sm:text-center pointer-events-none select-none px-4 sm:px-0">
        ...
        <div
          ...
          style={{
            ...
            fontWeight: 300,
            minHeight: '3.5em',
            color: theme === 'light' ? 'rgba(0,0,0,0.92)' : 'rgba(255,255,255,0.92)', // theme-appropriate text color
            textShadow: theme === 'light' 
              ? '0 2px 12px rgba(255,255,255,0.8), 0 1px 0 rgba(255,255,255,0.5)' // light glow for dark text on light background
              : '0 2px 12px rgba(0,0,0,0.32), 0 1px 0 #fff2', // subtle glow for readability on dark background
          }}
        >
          <ReactTyped
