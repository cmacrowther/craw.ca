...
export const projects: Project[] = [
  ...
  },
  {
    ...
    slug: "referee",
    title: "REFEREE",
    shortDescription: "Rust backend AI pipeline with a lightweight Tauri desktop app — hardware-accelerated upscaling, denoise, frame generation, and HDR tone mapping via NVIDIA and AMD SDKs.",
    longDescription:
      "REFEREE is a Rust backend pipeline with a lightweight Tauri desktop application compatible with Windows and Linux. It leverages NVIDIA RTX (NVEncC / RTX Video Super Resolution) and AMD Radeon (VCEEncC / AMD AI SDK) hardware-accelerated SDKs to deliver real-time AI upscaling, denoise, frame generation, and HDR tone mapping with color profiling — all on-device, without sending a single frame to a server. When vendor SDKs are unavailable or running on Linux, REFEREE falls back gracefully to ArtCNN ESRgan for upscaling, RIFE for frame generation, and Libplacebo for HDR tone mapping. Developers integrate with a few lines of JavaScript: serve a low-bitrate stream, and REFEREE intercepts it, enhances it through the pipeline, and hands back a high-quality HLS stream before the player notices. Available as a Windows and Linux desktop app via Tauri, and as self-hosted headless server distributables and Docker images for anyone interested in running it as a standalone server.",
    projectStory:
  ...
  },
  {
    ...
    longDescription:
      "REFEREE Docs is the Next.js documentation and marketing site built for the REFEREE application. It sells the value of REFEREE to end-users and developers alike: visitors can try upscaling firsthand through an interactive demo that streams a 480p source hosted by the site, then drag a comparison slider to reveal the REFEREE-upscaled output side by side. For developers interested in integrating their web streams or content with REFEREE, the site provides full API documentation covering how to make streams compatible with REFEREE's pipeline — giving their users access to AI-based upscaling, denoise, frame generation, and HDR tone mapping with color profiling.",
    projectStory:
      "Building a compelling case for REFEREE required more than a README. The pipeline's impact is visual — you have to see the difference to believe it — so the documentation site was designed around making that difference impossible to ignore. Hosting a real 480p stream and letting visitors drag a live slider between source and upscaled output puts the value proposition on the screen in seconds. The developer integration docs were built alongside the demo so that anyone convinced by what they saw could immediately find out how to bring that experience to their own platform and users.",
